...
let topicTag = document.getElementById('currentTopic');
function ensureTopicTag() {
  if (!topicTag) {
    topicTag = document.createElement('div');
    topicTag.id = 'currentTopic';
    topicTag.style.fontSize = '16px';
    topicTag.style.color = '#f2c94c';
    topicTag.style.opacity = '0.95';
    topicTag.style.marginLeft = '12px';
    topicTag.style.fontWeight = '700';
    if (topbar) {
      topbar.appendChild(topicTag);
    } else {
      const stage = document.querySelector('.stage');
      if (stage) stage.insertBefore(topicTag, stage.firstChild);
    }
  }
}
function updateTopicLabel() {
  ensureTopicTag();
  if (topicTag) topicTag.textContent = `Tema: ${currentTopic().name}`;
}
...
// === Reinicio de comodines (estado + UI) ===

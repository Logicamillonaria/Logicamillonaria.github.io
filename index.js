...
function init(){
  ...
  attachLifelines();
  bindIntroControls();
  startIntro(); // ⟵ mostramos intro con video
}
...
ensureStrokeDasharray();
function ensureStrokeDasharray(){ if (prog) prog.style.strokeDasharray = String(CIRC); }

function startTimer(){
  if (!state.canRunTimer) return; // no iniciar hasta desp de intro
...
function endGame(win){
  stopTimer();
  if (win){ triggerConfetti(); showMenu(`$${PRIZES[PRIZES.length-1]}`); }
  else {
    const clamped = Math.max(0, Math.min(PRIZES.length-1, state.idx));
...
btnRetry.addEventListener('click', ()=>{ hideMenu(); state.order = shuffle(state.order); startGame(); });
...
function attachLifelines(){
  ...
  if (b5050) b5050.onclick = ()=>{ if(state.usedLifelines['50']) return; state.usedLifelines['50']=true; use5050(); b5050.classList.add('used'); };
  if (bCall) bCall.onclick = ()=>{ if(state.usedLifelines['call']) return; state.usedLifelines['call']=true; useCall(); bCall.classList.add('used'); };
  if (bAud)  bAud.onclick = ()=>{ if(state.usedLifelines['aud']) return;  state.usedLifelines['aud']=true;  useAudience(); bAud.classList.add('used'); };
}
function use5050(){
...
function useAudience(){
  ...
  const qIdx = state.order[state.idx % state.order.length]; const correct = QUESTIONS[qIdx].correct;
  const nodes = [...answersEl.querySelectorAll('.answer')];
  nodes.forEach((n,i)=>{ const span=document.createElement('span'); span.textContent=(i===correct? Math.floor(50+Math.random()*30) : Math.floor(5+Math.random()*20)) + '%'; span.style.marginLeft='8px'; span.style.opacity='0.9'; n.appendChild(span); });
}

// Intro
function startIntro(){ introOverlay.style.display = 'flex'; try { introVideo.currentTime = 0; introVideo.play().catch(()=>{});} catch(_){} }
function closeIntro(){ introOverlay.style.display = 'none'; state.canRunTimer = true; startGame(); }
function bindIntroControls(){
  const tryPlay = ()=>{ try { introVideo.muted = false; introVideo.play().catch(()=>{});} catch(_){} };
  if (soundBtn) soundBtn.onclick = tryPlay;
  if (soundBtn2) soundBtn2.onclick = tryPlay;
...
function triggerConfetti(){
  ...
  wrap.innerHTML = '';
  for (let i=0;i<60;i++){
    const el=document.createElement('div'); el.style.position='absolute'; el.style.left=(Math.random()*100)+'%'; el.style.top='-10%'; el.style.width='10px'; el.style.height='16px'; el.style.background=`hsl(${Math.random()*360} 80% 60%)`; el.style.transform='rotate('+ (Math.random()*360)+'deg)'; el.style.opacity='0.95'; wrap.appendChild(el);
